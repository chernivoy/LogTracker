...
def create_text_window():
    ...
    # Создание стиля для тёмной темы
    dark_theme = ttk.Style()
    dark_theme.configure("Dark.TFrame", background="#333333", foreground="white")  # Пример цветов для тёмной темы
    dark_theme.configure("Dark.TButton", background="#666666", foreground="white", relief="flat")
    dark_theme.configure("Dark.TLabel", background="#333333", foreground="white")
    dark_theme.configure("Dark.TText", background="#333333", foreground="white")

    main_frame = ttk.Frame(root, style="Dark.TFrame")  # Применение стиля для основного фрейма
    main_frame.pack(fill="both", expand=True, padx=20, pady=20)
    ...
    file_label = ttk.Label(main_frame, font=("Arial", 12), anchor="w",
                           style="Dark.TLabel")  # Применение стиля для метки файла
    file_label.grid(row=0, column=0, padx=5, pady=5, sticky="nw")

    error_frame = ttk.Frame(main_frame, style="Dark.TFrame")  # Применение стиля для фрейма с ошибками
    error_frame.grid(row=1, column=0, columnspan=2, sticky="ew", pady=5)

    error_label = ttk.Label(error_frame, text="", anchor="w", style="Dark.TLabel")  # Применение стиля для метки ошибок
    error_label.pack(side="left", padx=(10, 0))

    error_text_widget_frame = ttk.Frame(error_frame,
                                        style="Dark.TFrame")  # Применение стиля для фрейма с текстовым виджетом ошибок
    error_text_widget_frame.pack(fill="both", expand=True)

    # Текстовый виджет с настройками цвета фона и переднего плана
    error_text_widget = tk.Text(error_text_widget_frame, height=4, wrap=tk.WORD, state=tk.DISABLED, bg="#333333",
                                fg="white")
    error_text_widget.pack(fill="both", expand=True, padx=5)

    text_widget_frame = ttk.Frame(main_frame,
                                  style="Dark.TFrame")  # Применение стиля для фрейма с основным текстовым виджетом
    text_widget_frame.grid(row=2, column=0, columnspan=2, sticky="nsew", pady=5)

    # Текстовый виджет с настройками цвета фона и переднего плана
    text_widget = tk.Text(text_widget_frame, wrap="none", bg="#333333", fg="white")
    text_widget.pack(fill="both", expand=True)
    ...
    icon_image = tk.PhotoImage(file="err_pic.png")
