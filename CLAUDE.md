...
### Показ помилок пачкою

`check_new_errors()` повертає **список** усіх помилок з нової частини файлу; `sync_files_and_check()` збирає їх по всіх файлах тіку в `{шлях: [рядки]}` і кладе в чергу **одне** сповіщення. `on_error_found(batch)` приймає саме цей словник.

Так зроблено, бо раніше з пачки виживала лише остання помилка (цикл перезаписував одну змінну), а при помилках у кількох файлах кожен клав свій колбек — на екрані лишався тільки останній файл.
