...
- ресайз за 8 напрямками — `change_cursor` / `start_resize` / `do_resize` / `stop_resize`. Зона краю `border = int(20 * scale)` — **масштабується за DPI** (20 фізичних px при DPI 200% давали б лише ~10 логічних, важко влучити трекпадом через RDP). Мінімум теж фізичний: `int(300*scale) × int(100*scale)`, щоб збігатися з `root.minsize(300,100)`, яку CTk теж множить, — інакше на межі OS клампив би ширину, а x усе одно зсувався б, і вікно «повзло» б. Масштаб кешується в `start_resize` (`_resize_scale`) на весь жест, `do_resize` не смикає Win32 на кожну подію і **не** друкує (раніше 6 `print` на подію роздували лог). Після `geometry()` `do_resize` робить `root.update_idletasks()` (застосувати відкладений re-layout — пересунути HWND кнопок до нового правого краю) і **завжди** форсує `RedrawWindow(RDW_INVALIDATE|RDW_ALLCHILDREN|RDW_UPDATENOW)`. Це критично для кнопок праворуч угорі: вони закріплені за правим краєм (`sticky "ne"`, колонка з `weight 0`), тож при ресайзі змінюється лише їхня **позиція**, а CTk перемальовує віджет тільки на зміні **розміру** (`_update_dimensions_event`) — пересунуті кнопки не перемальовуються й на шаруватому вікні зникають/стрибають до `stop_resize`. `update_idletasks` тут не досить: він **не** обробляє `WM_PAINT`, тому потрібен синхронний `RedrawWindow(...UPDATENOW)`. Додатковий `RDW_ERASE` вмикається **лише** на захід/північ (де рухається початок вікна): там контент їде екраном і майже-чорні згладжені краї тексту хедера (не збігаються з ключем `transparent_color="#000001"`, тож не стають прозорими) лишають слід — стирання фону його прибирає;
