...
- ресайз за 8 напрямками — `change_cursor` / `start_resize` / `do_resize` / `stop_resize`. Зона краю `border = int(20 * scale)` — **масштабується за DPI** (20 фізичних px при DPI 200% давали б лише ~10 логічних, важко влучити трекпадом через RDP). Мінімум теж фізичний: `int(300*scale) × int(100*scale)`, щоб збігатися з `root.minsize(300,100)`, яку CTk теж множить, — інакше на межі OS клампив би ширину, а x усе одно зсувався б, і вікно «повзло» б. Масштаб кешується в `start_resize` (`_resize_scale`) на весь жест, `do_resize` не смикає Win32 на кожну подію і **не** друкує (раніше 6 `print` на подію роздували лог). При ресайзі **на захід/північ** (де рухається початок вікна) `do_resize` після `geometry()` форсує `RedrawWindow(RDW_INVALIDATE|RDW_ERASE|RDW_ALLCHILDREN|RDW_UPDATENOW)`: інакше майже-чорні згладжені краї тексту хедера (не збігаються з ключем `transparent_color="#000001"`, тож не стають прозорими) розмазуються слідом за контентом і лишають темні артефакти. Для сходу/півдня початок не рухається — там перемальовка не потрібна;
