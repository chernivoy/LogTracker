...
- **Новий файл базується без сповіщень** (`_baseline_if_new`). `track_files()` на старті ставить офсет на кінець уже наявних файлів; файли, скопійовані пізніше, читалися з нуля, і вся їхня історія висипалася як свіжі помилки. На першому запуску так поводилися геть усі файли.
