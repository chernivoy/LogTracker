...
- **Новий файл береться на облік із межею за часом** (`_adopt_if_new`). Офсет ставиться на кінець, бо читання з нуля висипало б усю історію як свіжі помилки. Але з наявного вмісту показуються записи, новіші за `started_at`: стежений застосунок заводить **новий лог на кожну сесію**, і помилки, записані туди до першого погляду трекера, інакше зникали б безслідно.
...
Порівняння — за **міткою часу з самого рядка** (`parse_line_time()`, формат `2026.07.08 12.49.45.1094076`), а не за порядком обходу. Це принципово: `os.listdir()` віддає імена за абеткою, тож без розбору мітки свіжа помилка у файлі `mmm_*.log` програвала б старішій у `zzz_*.log`.

Для рядків без розпізнаваної мітки запасний ключ — `mtime` файлу. Грубо, але дозволяє змішувати логи різних форматів в одному тіку.
